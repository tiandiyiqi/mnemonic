...
将整个项目复制到 Agent 的 skills 目录即可：

```bash
# 复制 skills 目录到 Trae
mkdir -p ~/.trae/skills/
cp -r skills/universal-memory ~/.trae/skills/
...
# 复制 skills 到 Agent 目录
mkdir -p ~/.trae/skills/
cp -r mnemonic/skills/universal-memory ~/.trae/skills/
```
...
```bash
# 初始化全局记忆存储
python3 universal-memory/scripts/memory_cli.py init-global

# 在项目目录中初始化项目记忆
python3 universal-memory/scripts/memory_cli.py init-project

# 或指定项目路径
python3 universal-memory/scripts/memory_cli.py init-project --project-dir /path/to/project
```
...
```bash
# 全局偏好 (自动路由到全局)
python3 universal-memory/scripts/memory_cli.py remember "用户偏好函数式编程风格" --type preference

# 项目决策 (需要指定项目目录)
python3 universal-memory/scripts/memory_cli.py remember "API 采用 REST 架构" --type decision --project-dir /path/to/project

# 显式指定范围
python3 universal-memory/scripts/memory_cli.py remember "始终使用 TypeScript" --type constraint --scope global
python3 universal-memory/scripts/memory_cli.py remember "API 采用 REST 架构" --type decision --scope project --project-dir /path/to/project

# 带溯源元数据
python3 universal-memory/scripts/memory_cli.py remember "用户偏好简洁回复" --type preference --source user_dialog --confidence 0.9
```
...
```bash
# 自动搜索 (优先项目层，然后全局层)
python3 universal-memory/scripts/memory_cli.py search "数据库" --project-dir /path/to/project

# 仅搜索全局层
python3 universal-memory/scripts/memory_cli.py search "偏好" --scope global

# 仅搜索项目层
python3 universal-memory/scripts/memory_cli.py search "决策" --scope project --project-dir /path/to/project

# 搜索归档记忆
python3 universal-memory/scripts/memory_cli.py search "数据库" --include-archive --project-dir /path/to/project

# JSON 格式输出 (便于程序处理)
python3 universal-memory/scripts/memory_cli.py search "数据库" --json
```
...
```bash
python3 universal-memory/scripts/memory_cli.py reflect \
  --task "重构登录模块" \
  --outcome success \
  --confidence 0.9 \
...
```bash
python3 universal-memory/scripts/memory_cli.py perceive "用户正在询问关于登录模块的重构" --project-dir /path/to/project
```
...
```bash
python3 universal-memory/scripts/memory_cli.py remember "使用 PostgreSQL" --type decision --project-dir .
python3 universal-memory/scripts/memory_cli.py remember "改用 MongoDB" --type decision --project-dir .
# 第一条自动标记为 DEPRECATED
```
...
```bash
python3 universal-memory/scripts/memory_cli.py archive --project-dir .
